...
const int WAIT_TIME = 1000;             // waiting time in msecs
///////////////////////////////////////////////////////////
// helper macroses
...
///////////////////////////////////////////////////////////
// encoders
void QTWSClientSocket::EncodeField(QByteArray &os, const char* str)
{
    os.append(str);
    os.append('\0');
}

void QTWSClientSocket::EncodeField(QByteArray &os, const QString& string)
{
    os.append(string.toUtf8());
    os.append('\0');
}


void QTWSClientSocket::EncodeField(QByteArray &os, long longValue)
{
    char str[128];
...
void QTWSClientSocket::EncodeField(QByteArray& os, int intValue)
{
    char str[128];
...
void QTWSClientSocket::EncodeField(QByteArray& os, bool boolValue)
{
    int value = boolValue ? 1 : 0;
    EncodeField(os, value);
}

void QTWSClientSocket::EncodeField(QByteArray& os, double doubleValue)
{
	char str[128];
...
///////////////////////////////////////////////////////////
// "max" encoders
void QTWSClientSocket::EncodeFieldMax(QByteArray& os, int intValue)
{
    if( intValue == INT_MAX) {
        EncodeField(os, "");
        return;
...
void QTWSClientSocket::EncodeFieldMax(QByteArray& os, double doubleValue)
{
...
///////////////////////////////////////////////////////////
// decoders
bool QTWSClientSocket::CheckOffset(const char* ptr, const char* endPtr)
{
	assert (ptr && ptr <= endPtr);
	return (ptr && ptr < endPtr);
}

const char* QTWSClientSocket::FindFieldEnd(const char* ptr, const char* endPtr)
{
	return (const char*)memchr(ptr, 0, endPtr - ptr);
}

bool QTWSClientSocket::DecodeField(bool& boolValue, const char*& ptr, const char* endPtr)
{
	int intValue;
...
bool QTWSClientSocket::DecodeField(int& intValue, const char*& ptr, const char* endPtr)
{
...
bool QTWSClientSocket::DecodeField(long& longValue, const char*& ptr, const char* endPtr)
{
	int intValue;
	if( !DecodeField(intValue, ptr, endPtr))
		return false;
...
bool QTWSClientSocket::DecodeField(double& doubleValue, const char*& ptr, const char* endPtr)
{
	if( !CheckOffset(ptr, endPtr))
		return false;
	const char* fieldBeg = ptr;
...
bool QTWSClientSocket::DecodeField(QString& stringValue,
...
bool QTWSClientSocket::DecodeFieldMax(int& intValue, const char*& ptr, const char* endPtr)
{
    QString stringValue;
...
bool QTWSClientSocket::DecodeFieldMax(long& longValue, const char*& ptr, const char* endPtr)
{
	int intValue;
...
bool QTWSClientSocket::DecodeFieldMax(double& doubleValue, const char*& ptr, const char* endPtr)
{
    QString stringValue;
	if( !DecodeField(stringValue, ptr, endPtr))
		return false;
...
void QTWSClientSocket::CleanupBuffer(QByteArray& buffer, int processed)
{
    assert( buffer.isEmpty() || processed <= (int)buffer.size());
...
///////////////////////////////////////////////////////////
// utility funcs
static QString errMsg(std::exception e) {
...
///////////////////////////////////////////////////////////
// member funcs
QTWSClientSocket::QTWSClientSocket(QObject *parent)
    : QObject(parent)
...
QTWSClientSocket::~QTWSClientSocket()
{
    stop();
}
...
void QTWSClientSocket::onClose()
{
    if (m_connectionState != CS_SHUTDOWN) {
         eDisconnect();
         eReConnect();
    }else {
        eDisconnect();
...
void QTWSClientSocket::onReceive()
{
    checkMessages();
}

void QTWSClientSocket::socketError(QAbstractSocket::SocketError err)
{
    emit twsError(NO_VALID_ID, SOCKET_ERROR.code(), QString("%1 %2").arg(SOCKET_ERROR.msg())
                  .arg(m_socket.errorString()));
}

void QTWSClientSocket::eReConnect()
{
    if (m_connectionState == CS_SHUTDOWN || m_connectionState == CS_RECONNECTION)
        return;

    m_connectionState = CS_RECONNECTION;
    m_timer.start(3000);
...
bool QTWSClientSocket::eConnect()
{
    if( isConnected ()) {
        emit twsError( NO_VALID_ID, ALREADY_CONNECTED.code(), ALREADY_CONNECTED.msg());
        return false;
    }
    ...
    bool bConnected = m_socket.waitForConnected (m_waitTime);

    if (!bConnected) {
        //if(m_connectionState != CS_RECONNECTION)
            //emit twsError( NO_VALID_ID, CONNECT_FAIL.code(), CONNECT_FAIL.msg());
        goto exit_connect;
    }

    onConnectBase();

    m_socket.waitForBytesWritten(m_waitTime);

    while(!isConnected()) {
        m_socket.waitForReadyRead(m_waitTime);
        if ( !checkMessages()) {
            //emit twsError( NO_VALID_ID, CONNECT_FAIL.code(), CONNECT_FAIL.msg());
            goto exit_connect;
        }
...
void QTWSClientSocket::eDisconnect()
{
    if (isSocketOK()) {
        m_socket.close();
        emit twsConnectionClosed();
    }
    eDisconnectBase();
    m_connectionState = CS_IDLE;
}

bool QTWSClientSocket::start()
{
    bool isConnected =  eConnect();
    if (!isConnected) {
        eReConnect();
    }
    return isConnected;
...
void QTWSClientSocket::stop()
{
    m_connectionState = CS_SHUTDOWN;
    if(m_timer.isActive())
        m_timer.stop();
    eDisconnect();
}

bool QTWSClientSocket::isSocketOK() const
{
    return m_socket.isValid();
}

int QTWSClientSocket::send(const char* buf, size_t sz)
{
...
int QTWSClientSocket::receive(char* buf, size_t sz)
{
    return m_socket.read(buf, sz);
}

void QTWSClientSocket::eConnectBase()
{
}

void QTWSClientSocket::eDisconnectBase()
{
	Empty(m_TwsTime);
	m_serverVersion = 0;
	m_connected = false;
...
void QTWSClientSocket::reqMktData(long tickerId, const Contract& contract,
                               const QString& genericTicks, bool snapshot, const TagValueListSPtr& mktDataOptions)
{
	// not connected?
	if( !m_connected) {
        emit twsError( tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
		return;
	}

	// not needed anymore validation
	//if( m_serverVersion < MIN_SERVER_VER_SNAPSHOT_MKT_DATA && snapshot) {
    //	emit twsError( tickerId, UPDATE_TWS.code(), UPDATE_TWS.msg() +
	...
	{
        const Contract::ComboLegList* const comboLegs = contract.comboLegs.data();
		const int comboLegsCount = comboLegs ? comboLegs->size() : 0;
		ENCODE_FIELD( comboLegsCount);
		if( comboLegsCount > 0) {
			for( int i = 0; i < comboLegsCount; ++i) {
                const ComboLeg* comboLeg = ((*comboLegs)[i]).data();
				assert( comboLeg);
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
        QString mktDataOptionsStr("");
        const int mktDataOptionsCount = mktDataOptions.data() ? mktDataOptions->size() : 0;
		if( mktDataOptionsCount > 0) {
			for( int i = 0; i < mktDataOptionsCount; ++i) {
                const TagValue* tagValue = ((*mktDataOptions)[i]).data();
    ...
    bufferedSend( msg.constData(), msg.size());
}

void QTWSClientSocket::cancelMktData(long tickerId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqMktDepth(long tickerId, const Contract &contract, int numRows, const TagValueListSPtr& mktDepthOptions)
{
	// not connected?
	if( !m_connected) {
        emit twsError( tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
    ...
    //	emit twsError( NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg());
	//	return;
	//}
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
        QString mktDepthOptionsStr("");
        const int mktDepthOptionsCount = mktDepthOptions.data() ? mktDepthOptions->size() : 0;
		if( mktDepthOptionsCount > 0) {
			for( int i = 0; i < mktDepthOptionsCount; ++i) {
                const TagValue* tagValue = ((*mktDepthOptions)[i]).data();
...
void QTWSClientSocket::cancelMktDepth(long tickerId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqHistoricalData(long tickerId, const Contract &contract,
                                       ...
                                       int useRTH, int formatDate, const TagValueListSPtr& chartOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
        QString chartOptionsStr("");
        const int chartOptionsCount = chartOptions.data() ? chartOptions->size() : 0;
		if( chartOptionsCount > 0) {
			for( int i = 0; i < chartOptionsCount; ++i) {
                const TagValue* tagValue = ((*chartOptions)[i]).data();
...
void QTWSClientSocket::cancelHistoricalData(long tickerId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqRealTimeBars(long tickerId, const Contract &contract,
                                    int barSize, const QString &whatToShow, bool useRTH,
                                    const TagValueListSPtr& realTimeBarsOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
        QString realTimeBarsOptionsStr("");
        const int realTimeBarsOptionsCount = realTimeBarsOptions.data() ? realTimeBarsOptions->size() : 0;
		if( realTimeBarsOptionsCount > 0) {
			for( int i = 0; i < realTimeBarsOptionsCount; ++i) {
                const TagValue* tagValue = ((*realTimeBarsOptions)[i]).data();
...
void QTWSClientSocket::cancelRealTimeBars(long tickerId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqScannerParameters()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
		return;
	}
...
void QTWSClientSocket::reqScannerSubscription(int tickerId,
	const ScannerSubscription& subscription, const TagValueListSPtr& scannerSubscriptionOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
        QString scannerSubscriptionOptionsStr("");
        const int scannerSubscriptionOptionsCount = scannerSubscriptionOptions.data() ? scannerSubscriptionOptions->size() : 0;
		if( scannerSubscriptionOptionsCount > 0) {
			for( int i = 0; i < scannerSubscriptionOptionsCount; ++i) {
                const TagValue* tagValue = ((*scannerSubscriptionOptions)[i]).data();
...
void QTWSClientSocket::cancelScannerSubscription(int tickerId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( tickerId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqFundamentalData(long reqId, const Contract& contract,
                                           const QString& reportType)
{
	// not connected?
	if( !m_connected) {
        emit twsError( reqId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
		return;
	}
	...
	if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS) {
		if( contract.conId > 0) {
            emit twsError( reqId, UPDATE_TWS.code(), UPDATE_TWS.msg() +
...
void QTWSClientSocket::cancelFundamentalData(long reqId)
{
		// not connected?
	if( !m_connected) {
        emit twsError( reqId, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::calculateImpliedVolatility(long reqId, const Contract &contract, double optionPrice, double underPrice) {
...
void QTWSClientSocket::cancelCalculateImpliedVolatility(long reqId) {
...
void QTWSClientSocket::calculateOptionPrice(long reqId, const Contract &contract, double volatility, double underPrice) {
...
void QTWSClientSocket::cancelCalculateOptionPrice(long reqId) {
...
void QTWSClientSocket::reqContractDetails( int reqId, const Contract& contract)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqCurrentTime()
{
    // not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::placeOrder(long id, const Contract &contract, const Order &order)
{
	// not connected?
	if( !m_connected) {
        emit twsError( id, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
		return;
	}
    ...
    //		emit twsError( id, UPDATE_TWS.code(), UPDATE_TWS.msg() +
	...
	if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE && Compare(contract.secType, "BAG") == 0) {
        const Order::OrderComboLegList* const orderComboLegs = order.orderComboLegs.data();
		const int orderComboLegsCount = orderComboLegs ? orderComboLegs->size() : 0;
		for( int i = 0; i < orderComboLegsCount; ++i) {
            const OrderComboLeg* orderComboLeg = ((*orderComboLegs)[i]).data();
	...
	if( m_serverVersion >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS && Compare(contract.secType, "BAG") == 0) {
        const TagValueList* const smartComboRoutingParams = order.smartComboRoutingParams.data();
		...
		if( smartComboRoutingParamsCount > 0) {
			for( int i = 0; i < smartComboRoutingParamsCount; ++i) {
                const TagValue* tagValue = ((*smartComboRoutingParams)[i]).data();
	...
	if( m_serverVersion >= MIN_SERVER_VER_ALGO_ORDERS) {
		...
		if( !IsEmpty(order.algoStrategy)) {
            const TagValueList* const algoParams = order.algoParams.data();
			const int algoParamsCount = algoParams ? algoParams->size() : 0;
			ENCODE_FIELD( algoParamsCount);
			if( algoParamsCount > 0) {
				for( int i = 0; i < algoParamsCount; ++i) {
                    const TagValue* tagValue = ((*algoParams)[i]).data();
					ENCODE_FIELD( tagValue->tag);
					ENCODE_FIELD( tagValue->value);
				}
			}
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
        QString miscOptionsStr("");
        const TagValueList* const orderMiscOptions = order.orderMiscOptions.data();
		...
		if( orderMiscOptionsCount > 0) {
			for( int i = 0; i < orderMiscOptionsCount; ++i) {
                const TagValue* tagValue = ((*orderMiscOptions)[i]).data();
...
void QTWSClientSocket::cancelOrder(long id)
{
	// not connected?
	if( !m_connected) {
        emit twsError( id, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqAccountUpdates(bool subscribe, const QString& acctCode)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqOpenOrders()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqAutoOpenOrders(bool bAutoBind)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqAllOpenOrders()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqExecutions(int reqId, const ExecutionFilter& filter)
{
...
void QTWSClientSocket::reqIds( int numIds)
{
	// not connected?
	if( !m_connected) {
        emit twsError( numIds, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
		return;
	}
...
void QTWSClientSocket::reqNewsBulletins(bool allMsgs)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::cancelNewsBulletins()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::setServerLogLevel(int logLevel)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqManagedAccts()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::requestFA(faDataType pFaDataType)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::replaceFA(faDataType pFaDataType, const QString& cxml)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::exerciseOptions(long tickerId, const Contract &contract,
...
void QTWSClientSocket::reqGlobalCancel()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqMarketDataType( int marketDataType)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
int QTWSClientSocket::sendBufferedData()
{
    if( m_outBuffer.isEmpty())
		return 0;

    int nResult = send(m_outBuffer.constData(), m_outBuffer.size());
	if( nResult <= 0) {
		return nResult;
	}
...
int QTWSClientSocket::bufferedSend(const char* buf, size_t sz)
{
	if( sz <= 0)
		return 0;

    if( !m_outBuffer.isEmpty()) {
        m_outBuffer.append(buf, sz);
	...
	if( nResult < (int)sz) {
        ...
        m_outBuffer.append(buf + sent, sz);
	}

	return nResult;
}

int QTWSClientSocket::bufferedRead()
{
	char buf[8192];
    int nResult = receive(buf, sizeof(buf));

	if( nResult > 0) {
        m_inBuffer.append(&buf[0], nResult);
	}

	return nResult;
}

void QTWSClientSocket::reqPositions()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::cancelPositions()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::reqAccountSummary( int reqId, const QString& groupName, const QString& tags)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::cancelAccountSummary( int reqId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::verifyRequest(const QString& apiName, const QString& apiVersion)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::verifyMessage(const QString& apiData)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::queryDisplayGroups( int reqId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::subscribeToGroupEvents( int reqId, int groupId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::updateDisplayGroup( int reqId, const QString& contractInfo)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::startApi()
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void QTWSClientSocket::unsubscribeFromGroupEvents( int reqId)
{
	// not connected?
	if( !m_connected) {
        emit twsError( NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
bool QTWSClientSocket::checkMessages()
{
	if( !isSocketOK())
		return false;
    ...
    const char*	beginPtr = m_inBuffer.data();
...
int QTWSClientSocket::processConnectAck(const char*& beginPtr, const char* endPtr)
{
	...
	try {
        ...
        emit twsError( NO_VALID_ID, SOCKET_EXCEPTION.code(),
			SOCKET_EXCEPTION.msg() + errMsg( e) );
	}
	return 0;
}

int QTWSClientSocket::processMsg(const char*& beginPtr, const char* endPtr)
{
	...
	try {
		...
		switch( msgId) {
			case TICK_PRICE:
			{
                ...
                emit twsTickPrice( tickerId, (TickType)tickTypeInt, price, size, canAutoExecute);
                ...
                /*{
					...
					if( sizeTickType != NOT_SET)
                        emit twsTickSize( tickerId, sizeTickType, size);
                }*/
			...
			case TICK_SIZE:
			{
                ...
                emit twsTickSize( tickerId, (TickType)tickTypeInt, size);
				break;
			}
			...
			case TICK_OPTION_COMPUTATION:
			{
                ...
                emit twsTickOptionComputation( tickerId, (TickType)tickTypeInt,
			...
			case TICK_GENERIC:
			{
                ...
                emit twsTickGeneric( tickerId, (TickType)tickTypeInt, value);
				break;
			}
			...
			case TICK_STRING:
			{
                ...
                emit twsTickString( tickerId, (TickType)tickTypeInt, value);
				break;
			}
			...
			case TICK_EFP:
			{
				...
				int tickTypeInt;
				double basisPoints;
                QString formattedBasisPoints;
				double impliedFuturesPrice;
				int holdDays;
                QString futureExpiry;
                ...
                emit twsTickEFP( tickerId, (TickType)tickTypeInt, basisPoints, formattedBasisPoints,
			...
			case ORDER_STATUS:
			{
                ...
                emit twsOrderStatus( orderId, status, filled, remaining,
			...
			case ERR_MSG:
			{
                ...
                emit twsError( id, errorCode, errorMsg);
				break;
			}
			...
			case OPEN_ORDER:
			{
                ...
                emit twsOpenOrder( (OrderId)order.orderId, contract, order, orderState);
				break;
			}
			...
			case ACCT_VALUE:
			{
                ...
                emit twsUpdateAccountValue( key, val, cur, accountName);
				break;
			}
			...
			case PORTFOLIO_VALUE:
			{
                ...
                emit twsUpdatePortfolio( contract,
			...
			case ACCT_UPDATE_TIME:
			{
                ...
                emit twsUpdateAccountTime( accountTime);
				break;
			}
			...
			case CONTRACT_DATA:
			{
                ...
                emit twsContractDetails( reqId, contract);
				break;
			}
			...
			case BOND_CONTRACT_DATA:
			{
                ...
                emit twsBondContractDetails( reqId, contract);
				break;
			}
			...
			case EXECUTION_DATA:
			{
                ...
                emit twsExecDetails( reqId, contract, exec);
				break;
			}
			...
			case MARKET_DEPTH:
			{
                ...
                emit twsUpdateMktDepth( id, position, operation, side, price, size);
				break;
			}
			...
			case MARKET_DEPTH_L2:
			{
                ...
                emit twsUpdateMktDepthL2( id, position, marketMaker, operation, side,
					price, size);
			...
			case NEWS_BULLETINS:
			{
                ...
                emit twsUpdateNewsBulletin( msgId, msgType, newsMessage, originatingExch);
				break;
			}
			...
			case MANAGED_ACCTS:
			{
                ...
                emit twsManagedAccounts( accountsList);
				break;
			}
			...
			case RECEIVE_FA:
			{
                ...
                emit twsReceiveFA( (faDataType)faDataTypeInt, cxml);
				break;
			}
			...
			case HISTORICAL_DATA:
			{
                ...
                typedef QList<BarData> BarDataList;
				BarDataList bars;
				...
				for( int ctr = 0; ctr < itemCount; ++ctr) {
                    ...
                    emit twsHistoricalData( reqId, bar.date, bar.open, bar.high, bar.low,
						bar.close, bar.volume, bar.barCount, bar.average,
						Compare(bar.hasGaps, "true") == 0);
				}

				// send end of dataset marker
                QString finishedStr = QString("finished-") + startDateStr + "-" + endDateStr;
                emit twsHistoricalData( reqId, finishedStr, -1, -1, -1, -1, -1, -1, -1, 0);
				break;
			}
			...
			case SCANNER_DATA:
			{
                ...
                typedef QList<ScanData> ScanDataList;
				...
				for( int ctr=0; ctr < numberOfElements; ++ctr) {
                    ...
                    emit twsScannerData( tickerId, data.rank, data.contract,
						data.distance, data.benchmark, data.projection, data.legsStr);
				}

                emit twsScannerDataEnd( tickerId);
				break;
			}
			...
			case REAL_TIME_BARS:
			{
                ...
                emit twsRealtimeBar( reqId, time, open, high, low, close,
			...
			case FUNDAMENTAL_DATA:
			{
                ...
                emit twsFundamentalData( reqId, data);
				break;
			}
			...
			case CONTRACT_DATA_END:
			{
                ...
                emit twsContractDetailsEnd( reqId);
				break;
			}
			...
			case ACCT_DOWNLOAD_END:
			{
                ...
                emit twsAccountDownloadEnd( account);
				break;
			}
			...
			case DELTA_NEUTRAL_VALIDATION:
			{
                ...
                emit twsDeltaNeutralValidation( reqId, underComp);
				break;
			}
			...
			case MARKET_DATA_TYPE:
			{
                ...
                emit twsMarketDataType( reqId, marketDataType);
				break;
			}
			...
			case COMMISSION_REPORT:
			{
                ...
                emit twsCommissionReport( commissionReport);
				break;
			}
			...
			case POSITION_DATA:
			{
                ...
                emit twsPosition( account, contract, position, avgCost);
				break;
			}
			...
			case ACCOUNT_SUMMARY:
			{
                ...
                emit twsAccountSummary( reqId, account, tag, value, curency);
				break;
			}
			...
			case ACCOUNT_SUMMARY_END:
			{
                ...
                emit twsAccountSummaryEnd( reqId);
				break;
			}
			...
			case VERIFY_MESSAGE_API:
			{
                ...
                emit twsVerifyMessageAPI( apiData);
				break;
			}
			...
			case VERIFY_COMPLETED:
			{
                ...
                emit twsVerifyCompleted(bRes, errorText);
				break;
			}
			...
			case DISPLAY_GROUP_LIST:
			{
                ...
                emit twsDisplayGroupList( reqId, groups);
				break;
			}
			...
			case DISPLAY_GROUP_UPDATED:
			{
				int version;
				int reqId;
                QString contractInfo;

				DECODE_FIELD( version);
				DECODE_FIELD( reqId);
				DECODE_FIELD( contractInfo);

                emit twsDisplayGroupUpdated( reqId, contractInfo);
				break;
			}

			default:
			{
                emit twsError( msgId, UNKNOWN_ID.code(), UNKNOWN_ID.msg());
                eDisconnect();
				break;
			}
		}
...
///////////////////////////////////////////////////////////
// callbacks from socket
void QTWSClientSocket::onConnectBase()
{
	// send client version
    QByteArray msg;
...
bool QTWSClientSocket::isInBufferEmpty() const
{
    return m_inBuffer.isEmpty();
}

bool QTWSClientSocket::isOutBufferEmpty() const
{
    return m_outBuffer.isEmpty();
}
