...
typedef QSharedPointer<OrderComboLeg> OrderComboLegSPtr;
...
struct Order
{
	...
	// extended order fields
    QString tif;           // "Time in Force" - DAY, GTC, etc.
    QString activeStartTime;	// for GTC orders
    QString activeStopTime;	// for GTC orders
    QString ocaGroup;      // one cancels all group name
	int      ocaType;       // 1 = CANCEL_WITH_BLOCK, 2 = REDUCE_WITH_BLOCK, 3 = REDUCE_NON_BLOCK
    QString orderRef;      // order reference
	...
	bool     outsideRth;
	bool     hidden;
    QString goodAfterTime;    // Format: 20060505 08:00:00 {time zone}
    QString goodTillDate;     // Format: 20060505 08:00:00 {time zone}
    QString rule80A; // Individual = 'I', Agency = 'A', AgentOtherMember = 'W', IndividualPTIA = 'J', AgencyPTIA = 'U', AgentOtherMemberPTIA = 'M', IndividualPT = 'K', AgencyPT = 'Y', AgentOtherMemberPT = 'N'
	bool     allOrNone;
	int      minQty;
    ...
    QString faProfile;
    QString faMethod;
    QString faPercentage;

	// institutional (ie non-cleared) only
    QString openClose; // O=Open, C=Close
    ...
    QString designatedLocation; // set when slot=2 only.
	int      exemptCode;
    ...
    QString deltaNeutralOrderType;
	double   deltaNeutralAuxPrice;
	long     deltaNeutralConId;
    QString deltaNeutralSettlingFirm;
    QString deltaNeutralClearingAccount;
    QString deltaNeutralClearingIntent;
    QString deltaNeutralOpenClose;
	bool     deltaNeutralShortSale;
	int      deltaNeutralShortSaleSlot;
    QString deltaNeutralDesignatedLocation;
	...
	// HEDGE ORDERS
    QString hedgeType;  // 'D' - delta, 'B' - beta, 'F' - FX, 'P' - pair
    QString hedgeParam; // 'beta=X' value for beta hedge, 'ratio=Y' for pair hedge

	// Clearing info
    QString account; // IB account
    QString settlingFirm;
    QString clearingAccount; // True beneficiary of the order
    QString clearingIntent; // "" (Default), "IB", "Away", "PTA" (PostTrade)

	// ALGO ORDERS ONLY
    QString algoStrategy;
	...
	// order combo legs
    typedef QList<OrderComboLegSPtr> OrderComboLegList;
    typedef QSharedPointer<OrderComboLegList> OrderComboLegListSPtr;
...
Order::CloneOrderComboLegs(OrderComboLegListSPtr& dst, const OrderComboLegListSPtr& src)
{
	...
	for (; iter != iterEnd; ++iter) {
        const OrderComboLeg* leg = iter->data();
		if (!leg)
			continue;
...
Q_DECLARE_METATYPE(TWS::OrderComboLeg)
Q_DECLARE_METATYPE(TWS::Order)
